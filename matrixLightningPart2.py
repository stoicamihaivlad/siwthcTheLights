...
with open('input.txt', 'r',encoding='ISO-8859-1') as inputFile:
    for line in inputFile:
        ...
        if 'turn on' in line:
            ...
            turnOnV2(matrix,indexes[0],indexes[1],indexes[2],indexes[3])
            ...
            print(indexes)
            turnOffV2(matrix,indexes[0],indexes[1],indexes[2],indexes[3])
            print(f'Now are {countLightsOn(matrix)} lights On')
        elif 'toggle' in line:
           
            indexes = getRowsAndColumns(line)
            print(f'Switch the lights from {indexes[0]},{indexes[1]} through {indexes[2]},{indexes[3]}')
            toggleV2(matrix,indexes[0],indexes[1],indexes[2],indexes[3])
            print(f'Now are {countLightsOn(matrix)} lights On')
